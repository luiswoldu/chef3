...
def get_gaby_recipes_from_db():
    """
    Fetch recipes to enrich: only id >= 265 and url contains whatsgabycooking.com
    """
    page_size = 1000
    ...
    response = (
        supabase
        .table("recipes")
        .select("id,title,url,image,caption,tags")
        .gte("id", 265)
    ...
    print(f"Found {len(rows)} Gaby recipes to enrich")
    return rows
...
def main():
    recipes = get_gaby_recipes_from_db() 
